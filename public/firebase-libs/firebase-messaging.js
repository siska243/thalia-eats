...
const h = "@firebase/installations", w = "0.6.13", m = 1e4, y = `w:${w}`, b = "FIS_v2", v = 36e5,
    k = new ErrorFactory("installations", "Installations", {
...
async function set(e, t) {
    const n = getKey$1(e), r = (await getDbPromise$1()).transaction(E, "readwrite"), a = r.objectStore(E),
        o = await a.get(n);
    return await a.put(t, n), await r.done, o && o.fid === t.fid || fidChanged(e, t.fid), t
}

async function remove(e) {
    const t = getKey$1(e), n = (await getDbPromise$1()).transaction(E, "readwrite");
    ...
    const n = await update(e.appConfig, (n => {
        const r = function updateOrCreateInstallationEntry(e) {
            ...
            if (0 === t.registrationStatus) {
                if (!navigator.onLine) {
                    return {installationEntry: t, registrationPromise: Promise.reject(k.create("app-offline"))}
                }
                    ...
                    r = async function registerInstallation(e, t) {
                        try {
                            const n = await async function createInstallationRequest({
                                ...
                                const i = {fid: n, authVersion: b, appId: e.appId, sdkVersion: y},
...
async function generateAuthTokenRequest({appConfig: e, heartbeatServiceProvider: t}, n) {
    ...
    const i = {installation: {sdkVersion: y, appId: e.appId}},
...
async function refreshAuthToken(e, t = !1) {
    ...
    const r = await update(e.appConfig, (r => {
        if (!isEntryRegistered(r)) throw k.create("not-registered");
        const a = r.authToken;
...
function updateAuthTokenRequest(e) {
    return update(e, (e => {
        if (!isEntryRegistered(e)) throw k.create("not-registered");
...
const C = "installations", publicFactory = e => {
    ...
    const t = e.getProvider("app").getImmediate(), n = _getProvider(t, C).getImmediate();
    return {
...
const B = "fcm_token_details_db", $ = "fcm_token_object_Store";
const R = "firebase-messaging-store";
let F = null;

function getDbPromise() {
    return F || (F = openDB("firebase-messaging-database", 1, {
        upgrade: (e, t) => {
...
async function dbGet(e) {
    const t = getKey(e), n = await getDbPromise(), r = await n.transaction(R).objectStore(R).get(t);
    if (r) return r;
    {
        const t = await async function migrateOldDatabase(e) {
            if ("databases" in indexedDB) {
                const e = (await indexedDB.databases()).map((e => e.name));
                if (!e.includes(B)) return null
            }
            let t = null;
            return (await openDB(B, 5, {
                upgrade: async (n, r, a, o) => {
                    var i;
                    if (r < 2) return;
                    if (!n.objectStoreNames.contains($)) return;
                    const s = o.objectStore($), c = await s.index("fcmSenderId").get(e);
            ...
            })).close(), await deleteDB(B), await deleteDB("fcm_vapid_details_db"), await deleteDB("undefined"), function checkTokenDetails(e) {
...
async function dbSet(e, t) {
    const n = getKey(e), r = (await getDbPromise()).transaction(R, "readwrite");
    return await r.objectStore(R).put(t, n), await r.done, t
}
...
const K = new ErrorFactory("messaging", "Messaging", {
    "missing-app-config-values": 'Missing App configuration value: "{$valueName}"',
...
async function requestDeleteToken(e, t) {
    ...
    try {
        ...
        if (a.error) {
            ...
            throw K.create("token-unsubscribe-failed", {errorInfo: e})
        }
    } catch (e) {
        throw K.create("token-unsubscribe-failed", {errorInfo: null == e ? void 0 : e.toString()})
    }
}
            ...
            try {
                const n = await async function requestUpdateToken(e, t) {
                        ...
                        try {
                            ...
                            throw K.create("token-update-failed", {errorInfo: null == e ? void 0 : e.toString()})
                        }
                        if (o.error) {
                            const e = o.error.message;
                            throw K.create("token-update-failed", {errorInfo: e})
                        }
                        if (!o.token) throw K.create("token-update-no-token");
                        return o.token
...
async function deleteTokenInternal(e) {
    ...
    t && (await requestDeleteToken(e.firebaseDependencies, t.token), await async function dbRemove(e) {
        const t = getKey(e), n = (await getDbPromise()).transaction(R, "readwrite");
...
async function getNewToken(e, t) {
    const n = await async function requestGetToken(e, t) {
        ...
        try {
            ...
            throw K.create("token-subscribe-failed", {errorInfo: null == e ? void 0 : e.toString()})
        }
        if (o.error) {
            const e = o.error.message;
            throw K.create("token-subscribe-failed", {errorInfo: e})
        }
        if (!o.token) throw K.create("token-subscribe-no-token");
        return o.token
...
async function registerDefaultSw(e) {
    try {
        ...
        })), await async function waitForRegistrationActive(e) {
        ...
        throw K.create("failed-service-worker-registration", {browserErrorMessage: null == e ? void 0 : e.message})
    }
}

async function getToken$1(e, t) {
    if (!navigator) throw K.create("only-available-in-window");
    ...
    return await async function updateVapidKey(e, t) {
        ...
        if (t || e.swRegistration || await registerDefaultSw(e), t || !e.swRegistration) {
            if (!(t instanceof ServiceWorkerRegistration)) throw K.create("invalid-sw-registration");
            e.swRegistration = t
        }
...
async function messageEventListener(e, t) {
    ...
    e.onMessageHandler && n.messageType === j.PUSH_RECEIVED && ("function" == typeof e.onMessageHandler ? e.onMessageHandler(externalizePayload(n)) : e.onMessageHandler.next(externalizePayload(n)));
    const r = n.data;
...
const x = "@firebase/messaging", q = "0.12.17", WindowMessagingFactory = e => {
...
function getMessagingInWindow(e = n()) {
    return isWindowSupported().then((e => {
        if (!e) throw K.create("unsupported-browser")
    }), (e => {
        throw K.create("indexed-db-unsupported")
...
!function registerMessagingInWindow() {
    t(new Component("messaging", WindowMessagingFactory, "PUBLIC")), t(new Component("messaging-internal", WindowMessagingInternalFactory, "PRIVATE")), e(x, q), e(x, q, "esm2017")
}();
